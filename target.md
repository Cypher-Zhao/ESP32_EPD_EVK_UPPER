...
## B.待修复
- [x] 串口通讯和tcp通讯同时只会启用一个，一起连上后另一个会禁用，只是禁止连接这个按钮，希望能够禁止专跳到另一个通讯方式页面
- [x] 控制台和主页切换的相当于是转跳了页面，会丢失连接状态。控制台在当当前网页里实现就行，不用另起一页
- [x] 即使控制台里已显示连接上了，但是消息发送框仍然被禁用
- [x] 串口连接页面中，刷新按钮风格格格不入，改成连接和断开的风格就行，且不需要图片